...
class WorkspaceFS:
    ...
    async def list_dir(
        ...
        async for objects in _metadata_pages(self._storage, directory_prefix):
            for item in objects:
                ...
                if public_path not in entries and len(entries) == MAX_DIRECTORY_ENTRIES:
                    raise WorkspaceError(
                        ErrorCode.WORKSPACE_DIRECTORY_TOO_LARGE,
                        "Workspace directory has too many entries to list",
                    )
                if separator:
                    entries[public_path] = DirectoryEntry(
                        path=public_path,
                        is_directory=True,
                        size=None,
                    )
                elif public_path not in entries:
                    entries[public_path] = DirectoryEntry(
                        path=public_path,
                        is_directory=False,
                        size=item.size,
        ...
        if relative_path and not entries:
            try:
    ...
    async def _write_locked(
        ...
        *,
        ...
        return FileMetadata(size=len(data), etag=uploaded.etag)
