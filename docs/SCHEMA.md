...
- `delivery_refs` — JSONB array of user-visible file delivery references for channel adapters, ignored by provider replay. Py5's web `message(media=...)` uses this sidecar for file chips/download links so `messages.content` can stay Anthropic-compatible. Each ref links to its generating `tool_use_id`. Server workspace refs are durable and retain the virtual path plus immutable workspace ID/workspace-relative path so a frontend can recover from a shared-workspace rename. Device refs are online-only pointers to `(device name, path)`; the browser resolves them later through the Workspace Files `GET` route and may receive `tool_device_unreachable`, `workspace_not_found`, or policy errors at click time. Device refs do not cause a device read or RustFS write when the message is sent.
...
- Py5 has no shell, environment, command-policy, or MCP configuration columns.
  Future milestones must define any such fields together with their runtime
  implementation; they are not persisted or sent by the Py5 client.
